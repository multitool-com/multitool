...
export default function YamlFormatterClient() {
  ...
  const run = (mode: "format" | "validate") => {
    setError("");
    setOk("");
    setCopied(false);
    if (!input.trim()) {
    ...
    try {
      ...
      trackToolUsed("yaml-formatter", "developer-tools");
      setOutput(stringify(obj, { indent: indent === "2" ? 2 : 4, lineWidth: 0 }));
    } catch (e) {
  ...
  const copy = async () => {
    ...
    try {
      ...
      setCopied(true);
      trackCopy("yaml-formatter", "developer-tools");
    ...
    <div className="flex flex-col gap-5">
      <div className="bg-white border border-ink/10 rounded-xl p-6 shadow-sm">
        ...
        <textarea
          ...
          rows={8}
          spellCheck={false}
          placeholder={"# docker-compose, k8s manifest, CI config…\nservices:\n  web:\n    image: nginx"}
